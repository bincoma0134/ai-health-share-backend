...
class _ModeratorDashboardScreenState extends State<ModeratorDashboardScreen> {
  ...
  void _showModerateModal(Map<String, dynamic> item, bool isHistory) {
    ...
    _isProcessing = false; // Reset lại vòng xoay mỗi khi mở Modal
                      ...
                      Row(
                        children: [
                          ...
                          Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text(item['author']?['full_name'] ?? 'Ẩn danh', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                              Text('Tác giả', style: const TextStyle(color: Colors.white54, fontSize: 10)),
                            ],
                          ),
                        ],
                      ),
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(item['description'] ?? 'Không có mô tả', style: const TextStyle(color: Colors.white70, fontSize: 12), maxLines: 3, overflow: TextOverflow.ellipsis),
                      
                      // Hiển thị Ghi chú nếu là Lịch sử
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(item['moderation_note'], style: const TextStyle(color: Colors.white, fontStyle: FontStyle.italic)),
                            ],
                          ),
                        )
                      ],

                      // Ô nhập lý do từ chối (Chỉ ở Queue)
                      if (showRejectInput && !isHistory) ...[
                        const SizedBox(height: 16),
                        TextField(
                          controller: noteCtrl,
                          style: const TextStyle(color: Colors.white),
                          decoration: InputDecoration(hintText: 'Nhập lý do từ chối (Bắt buộc)...', filled: true, fillColor: _modDanger.withOpacity(0.1), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: _modDanger.withOpacity(0.5)))),
                        ),
                      ],
                    ],
                  ),
                    ...
                    child: isPendingDelete 
                      ? Row(
                          children: [
                            ...
                            const SizedBox(width: 12),
                            Expanded(child: _buildActionButton('TỪ CHỐI XÓA', Colors.grey, _isProcessing, () => _handleAction(item, 'APPROVED', noteCtrl.text, setModalState))),
                          ],
                        )
                      : Row(
                          children: [
                            Expanded(
                              child: showRejectInput 
                                ...
                                : _buildActionButton('YÊU CẦU SỬA', _modDanger.withOpacity(0.2), _isProcessing, () => setModalState(() => showRejectInput = true), textColor: _modDanger),
                            ),
                            const SizedBox(width: 12),
                            Expanded(child: _buildActionButton('PHÊ DUYỆT', _modPrimary, _isProcessing, () => _handleAction(item, 'APPROVED', '', setModalState), disabled: showRejectInput)),
                          ],
                        ),
                  )
              ],
  ...
  Future<void> _handleAction(Map<String, dynamic> item, String action, String note, Function setModalState) async {
    ...
    if (success && mounted) {
      ...
      _loadAllData();
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Đã $action thành công!'), backgroundColor: Colors.green));
    } else {
      setModalState(() => _isProcessing = false);
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Lỗi xử lý!'), backgroundColor: Colors.red));
    }
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Row(
          children: [
            ...
            const Text('Moderator Workspace', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w900)),
          ],
        ),
        actions: [
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: _isLoading 
              ? Center(child: CircularProgressIndicator(color: _modPrimary))
              : RefreshIndicator(
                  color: _modPrimary,
    ...
    return Column(
      ...
      children: [
        ...
        Container(
          width: double.infinity,
          padding: const EdgeInsets.all(24),
          decoration: BoxDecoration(gradient: LinearGradient(colors: [_modPrimary, _modPrimary.withOpacity(0.5)]), borderRadius: BorderRadius.circular(24)),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
        ...
        const Text('BIỂU ĐỒ KIỂM DUYỆT (7 NGÀY)', style: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w900, letterSpacing: 1)),
        const SizedBox(height: 16),
            ...
            LineChartData(
              ...
              titlesData: FlTitlesData(
                ...
                bottomTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, reservedSize: 22, getTitlesWidget: (val, meta) {
                  if (val.toInt() >= 0 && val.toInt() < chartData.length) return Text(chartData[val.toInt()]['date'], style: const TextStyle(color: Colors.white54, fontSize: 10));
                  return const SizedBox();
                })),
              ),
              borderData: FlBorderData(show: false),
              lineBarsData: [
                LineChartBarData(spots: duyetSpots, isCurved: true, color: _modPrimary, barWidth: 3, dotData: const FlDotData(show: false), belowBarData: BarAreaData(show: true, color: _modPrimary.withOpacity(0.2))),
                LineChartBarData(spots: tuChoiSpots, isCurved: true, color: _modDanger, barWidth: 3, dotData: const FlDotData(show: false), belowBarData: BarAreaData(show: true, color: _modDanger.withOpacity(0.2))),
              ],
            ),
          ),
        ),
  ...
  Widget _buildQueueTab() {
    ...
    return Column(
      children: [
        SingleChildScrollView(
          ...
          child: Row(
            children: [
              ...
              {'id': 'video', 'label': 'Video'}, {'id': 'delete', 'label': 'Gỡ bỏ'}, {'id': 'edit', 'label': 'Cập nhật'}
            ].map((f) {
              final isActive = _queueFilter == f['id'];
              return Padding(
                padding: const EdgeInsets.only(right: 8),
                child: ChoiceChip(
                  label: Text(f['label']!, style: TextStyle(color: isActive ? Colors.black : Colors.white, fontWeight: FontWeight.bold, fontSize: 12, letterSpacing: 0.5)),
                  selected: isActive,
        ...
        if (filtered.isEmpty)
          const Padding(padding: EdgeInsets.only(top: 40), child: Center(child: Text('Không có mục nào trong hàng đợi.', style: TextStyle(color: Colors.white54))))
        else
            ...
            child: Container(
              ...
              child: Row(
                children: [
                  Container(
                    width: 60, height: 60,
                    decoration: BoxDecoration(color: Colors.black, borderRadius: BorderRadius.circular(12), image: item['image_url'] != null ? DecorationImage(image: NetworkImage(item['image_url']), fit: BoxFit.cover) : null),
                    child: (item['image_url'] == null && item['video_url'] != null) ? const Icon(Icons.play_circle, color: Colors.white) : null,
                  ),
                  const SizedBox(width: 12),
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 4),
                        Text('Tác giả: ${item['author']?['full_name'] ?? 'Ẩn danh'}', style: const TextStyle(color: Colors.white54, fontSize: 10)),
                      ],
                    ),
                  ),
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                    decoration: BoxDecoration(color: item['status'].toString().contains('DELETE') ? _modDanger.withOpacity(0.2) : Colors.amber.withOpacity(0.2), borderRadius: BorderRadius.circular(8)),
                    child: Text(item['status'].toString().split('_').last, style: TextStyle(color: item['status'].toString().contains('DELETE') ? _modDanger : Colors.amber, fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 0.5)),
                  )
                ],
              ),
            ),
  ...
  Widget _buildHistoryTab() {
    if (_history.isEmpty) return const Center(child: Text('Chưa có lịch sử xử lý.', style: TextStyle(color: Colors.white54)));
    ...
    return Column(
      children: _history.map((item) => GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(color: Colors.white.withOpacity(0.02), border: Border(left: BorderSide(color: item['status'] == 'APPROVED' ? Colors.green : _modDanger, width: 4)), borderRadius: BorderRadius.circular(12)),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Expanded(child: Text(item['title'] ?? '', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold), maxLines: 1, overflow: TextOverflow.ellipsis)),
              ...
              Text(item['moderation_note'] ?? 'Không có ghi chú', style: const TextStyle(color: Colors.white54, fontSize: 12, fontStyle: FontStyle.italic), maxLines: 1, overflow: TextOverflow.ellipsis),
            ],
          ),
        ),
      )).toList(),
  ...
  Widget _buildTab(String key, String title, IconData icon) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: isActive ? _modPrimary : Colors.white.withOpacity(0.05), borderRadius: BorderRadius.circular(20)),
        child: Row(
          children: [
            Icon(icon, size: 16, color: isActive ? Colors.white : Colors.white54),
            const SizedBox(width: 8),
            Text(title.toUpperCase(), style: TextStyle(color: isActive ? Colors.white : Colors.white54, fontWeight: FontWeight.bold, fontSize: 10, letterSpacing: 1)),
          ],
        ),
      ),
    );
